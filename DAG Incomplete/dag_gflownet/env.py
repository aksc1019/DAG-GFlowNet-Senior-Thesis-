...
class GFlowNetDAGEnv(gym.vector.VectorEnv):
    ...
    def calculate_total_bde_score(self):
        total_scores = []
    
        # Iterate through each environment
        for env_index in range(self.num_envs):
            # Access the adjacency matrix for the current environment
            adjacency_matrix = self._state['adjacency'][env_index]
            num_nodes = self.num_variables
            env_total_score = 0  # Initialize total score for this environment
        
            for target in range(num_nodes):
                # Collect indices where there is an edge leading to 'target'
                indices = np.where(adjacency_matrix[:, target] == 1)[0]
                if indices.size > 0:
                    # Get local scores for the target node with its parents
                    _, local_score_after = self.scorer.get_local_scores(target, indices.tolist())
                    # Sum up the local scores for this environment
                    env_total_score += local_score_after.score
        
            # Append the total score for this environment to the list of total scores
            total_scores.append(env_total_score)
    
        # Calculate the average score across all environments, if total_scores is not empty
...
#  #IGNORE:   # New -- sums local scores of edges of DAGs
#     def calculate_total_score(self):
#         total_score = 0
#         # Assuming self._state['adjacency'] is the adjacency matrix representation of the DAG
#         adjacency_matrix = self._state['adjacency']
#         num_nodes = adjacency_matrix.shape[0]

#         for source in range(num_nodes):
#             for target in range(num_nodes):
#                 if adjacency_matrix[source, target] == 1:
#                     # Assuming get_local_score method calculates the score for the edge from source to target
#                     edge_score = self.scorer.get_local_score(source, target)
...
#     # New -- calculates total BDe score
#     def calculate_total_bde_score(self):
#         total_score = 0
#         # Assuming self._state['adjacency'] represents the DAG's adjacency matrix
#         adjacency_matrix = self._state['adjacency']
#         num_nodes = self.num_variables

# #        # Diagnostic for debugging
# #        print("Shape of adjacency_matrix:", adjacency_matrix.shape)
# #        print("Type of adjacency_matrix:", type(adjacency_matrix))
# #        # Ensure it's a 2D NumPy array
# #        assert isinstance(adjacency_matrix, np.ndarray) and adjacency_matrix.ndim == 2, "adjacency_matrix must be a 2D NumPy array"
        
#         for target in range(num_nodes):
#             indices = [i for i in range(num_nodes) if adjacency_matrix[i, target] == 1]
#             # Also tried using .any() or .all() in case adjacency_matrix[i, target] doesn't give me a single (Boolean) value... but doesn't seem to work either.
#             # Alternative try (2) that didn't work:           indices = np.where(adjacency_matrix[:, target] == 1)[0]
#             if indices:             # alternative (2): if indices.size > 0:
#                 # Get local scores for the target node with its parents
#                 _, local_score_after = self.scorer.get_local_scores(target, indices)
#                 # Sum up the local scores
#                 total_score += local_score_after.score
